...
| 12 | **⏰ REMINDER — remove the `best-seller` tag before launch** | Kept deliberately for now. Development and production read the **same** Shopify store, so there is no test copy: the tag is live data. The Best Seller badge renders only on the "you may also like" cards (`suggested-product-card.tsx`), so with a single product it shows nowhere today — it **will appear on the live site as soon as a second product exists**, and `/product/...` pages are publicly reachable during coming-soon. To preview badges without touching real data, set `VITE_USE_MOCK_PRODUCTS=true` in `client/.env.local` — the mock catalog has Best Seller, New and Limited items. Badges are manual Shopify tags: `best-seller`, `new`, `limited`. Use only when true; `limited` never as urgency |
