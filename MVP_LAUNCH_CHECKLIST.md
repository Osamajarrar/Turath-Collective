...
| 12 | **Remove the `best-seller` tag in Shopify** | The Indigo Mosaic Bowl carries it, which renders a "Best Seller" badge (`suggested-product-card.tsx`) on a store that has never sold anything. `/shop` and `/product/...` are publicly reachable during coming-soon. Hard rule 4. Re-add only when true |
...
| 14 | **DMARC — anti-spoofing** | The Cloudflare banner. Cloudflare → Email → DMARC Management → Enable. Monitor-only first; after ~2 weeks of reports showing only Resend and Cloudflare, move to `p=quarantine`. Resend signs with the domain’s own key, so its mail passes |
| 15 | **Real-user analytics** | Known bots are already excluded: GA4 always, PostHog by default (not disabled in `analytics.ts`). Most AI crawlers never run JS so never load analytics at all. Remaining: exclude the founder’s own traffic (PostHog internal-user filter; GA4 internal-traffic rule by IP), and decide deliberately whether to block AI crawlers at Cloudflare — recommendation is not to, since it also removes the brand from AI search answers. Agentic browsers (ChatGPT agent, Perplexity Comet) look human and cannot be reliably filtered yet |

## Blocking launch — needs code

| # | Item | Notes |
|---|---|---|
| 8 | **Category source of truth** | `inferCategoryHandle` ([client/src/pages/shop.tsx:148](client/src/pages/shop.tsx#L148)) guesses from Shopify Type, falling back to the first tag, keyword-matching "bowl"/"plate"/"mug" to ceramics and defaulting anything empty to ceramics. The one live product has Type `Bowl` — a shape, not a craft — and is filed correctly by luck. **Plan:** founder sets Shopify **Type** (not the separate Category field) to the craft, `Ceramics` or `Glass`, on every product; code then reads Type only, requires an exact match against the registry in `client/src/lib/collections.ts`, and shows anything unmatched under All only, reported rather than guessed. **This is the one expensive-to-reverse item left** |
