...
| `RESEND_NOTIFY_AUDIENCE_ID` | Resend audience for "tell me when this piece is available". A **separate CASL consent** from the newsletter — never merge the two |
...
- `npm run dev` = Express + Vite middleware on **port 5000** — local only. `server/` no longer
  ...
  to check headers, the SPA fallback and API behaviour. `npm run check` = typecheck (keep it
  passing — regressions hide there); `npm test` = vitest; `npm run build` before assuming a
  Vite-config change works.
- The self-hosted auth stack (`server/auth.ts`, `server/storage.ts`, `server/admin.ts`, passport,
  express-session), the unrouted auth/checkout/admin pages, and the file-backed JSON stores are
  **deleted** (plan 10 phases 6b–6c). Shopify's Customer Account API owns accounts. Still deferred
  and intentional: the commented tables in `shared/schema.ts` (now **D1/SQLite** dialect, no
  database provisioned) and `shopifyService.buyNow` (unused — wiring it would bypass the
  checkout-intent dialog and land users on the storefront password page). Don't "clean up" what a
  comment marks as deferred; don't wire it live either.
